...
model = hub.Module(
    name='roberta-wwm-ext-large', version='2.0.2', task='text-matching')
...
trainer = hub.Trainer(
    model,
    optimizer,
    checkpoint_dir=os.path.join(models_save, 'roberta-wwm-ext-large'),
    use_gpu=True)
...
trainer.train(
    ...
    save_interval=1, )
trainer.evaluate(test_dataset, batch_size=8)
