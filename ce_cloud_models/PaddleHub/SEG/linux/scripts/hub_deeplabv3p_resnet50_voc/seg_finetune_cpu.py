...
/***************************************************************************
  ...
  * @file:  seg_finetune_cpu.py
  * @date  2021/5/8 1:27 PM
  * @brief 
  *
  **************************************************************************/
"""
...
model = hub.Module(name='deeplabv3p_resnet50_voc', num_classes=2)
...
trainer = Trainer(
    model,
    optimizer,
    checkpoint_dir=os.path.join(models_save, 'deeplabv3p_resnet50_voc'),
    use_gpu=False)
trainer.train(
    train_reader, epochs=2, batch_size=8, log_interval=1, save_interval=1)
