...
if [ "$1" = "linux_st_gpu1" ];then #单卡
    ...
    --data "imagenet" \
    --model_path "./fpgm_mobilenetv1_models_gpu1/0" > ${log_path}/$2.log 2>&1
    print_info $? $2

elif [ "$1" = "linux_st_gpu2" ];then #单卡
    python eval.py --model "MobileNet" \
    --data "imagenet" \
    --model_path "./fpgm_mobilenetv1_models_gpu2/0" > ${log_path}/$2.log 2>&1
    print_info $? $2

elif [ "$1" = "linux_st_cpu" ];then #单卡
    python eval.py --model "MobileNet" \
    --data "imagenet" \
    --model_path "./fpgm_mobilenetv1_models_cpu/0"  \
