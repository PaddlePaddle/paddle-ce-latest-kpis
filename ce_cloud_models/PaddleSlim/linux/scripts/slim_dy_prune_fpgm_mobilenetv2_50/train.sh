...
if [ "$1" = "linux_dy_gpu1" ];then #单卡
    ...
    --criterion="fpgm" \
    --model_path="./fpgm_mobilenetv2_models" > ${log_path}/$2.log 2>&1
    print_info $? $2
elif [ "$1" = "linux_dy_gpu2" ];then # 多卡
    python -m paddle.distributed.launch \
    --log_dir="fpgm_mobilenetv2_50_linux_dy_gpu2_dist_log" train.py \
    --model="mobilenet_v2" \
    --data="imagenet" \
    --pruned_ratio=0.3125 \
    --lr=0.1 \
    ...
    --model_path="./fpgm_mobilenetv2_models" > ${log_path}/$2.log 2>&1
    print_info $? $2
    mv $code_path/fpgm_mobilenetv2_50_linux_dy_gpu2_dist_log $log_path/$2_dist_log
