...
if [ "$1" = "linux_dy_gpu1_con" ];then #单卡
    ...
    sed -i "s/  use_gpu: False/  use_gpu: True/g" config_bigdata.yaml
    python -u ../../../tools/static_trainer.py -m config_bigdata.yaml -o > ${log_path}/$2.log 2>&1
    print_info $? $2
fi
...
if [ "$1" = "linux_dy_gpu1" ];then #单卡
    python -u ../../../tools/trainer.py -m config_bigdata.yaml \
     -o runner.model_save_path="output_model_textcnn_all_dy_gpu1" runner.use_gpu=True > ${log_path}/$2.log 2>&1
    print_info $? $2
elif [ "$1" = "linux_dy_gpu2" ];then #多卡
    # 多卡的运行方式
    python -m paddle.distributed.launch ../../../tools/trainer.py -m config_bigdata.yaml \
     -o runner.model_save_path="output_model_textcnn_all_dy_gpu2" runner.use_gpu=True runner.use_fleet=True >${log_path}/$2.log 2>&1
    print_info $? $2
    mv $code_path/log $log_path/$2_dist_log
elif [ "$1" = "linux_dy_cpu" ];then
    python -u ../../../tools/trainer.py -m config_bigdata.yaml \
     -o runner.model_save_path="output_model_textcnn_all_dy_cpu" runner.use_gpu=False > ${log_path}/$2.log 2>&1
    print_info $? $2

elif [ "$1" = "linux_st_gpu1" ];then #单卡
    python -u ../../../tools/static_trainer.py -m config_bigdata.yaml \
     -o runner.model_save_path="output_model_textcnn_all_st_gpu1" runner.use_gpu=True > ${log_path}/$2.log 2>&1
    print_info $? $2
elif [ "$1" = "linux_st_gpu2" ];then #多卡
    sed -i "s/  use_gpu: False/  use_gpu: True/g" config_bigdata.yaml
    # 多卡的运行方式
    python -m paddle.distributed.launch ../../../tools/static_trainer.py -m config_bigdata.yaml \
     -o runner.model_save_path="output_model_textcnn_all_st_gpu2" runner.use_gpu=True runner.use_fleet=True >${log_path}/$2.log 2>&1
    print_info $? $2
    mv $code_path/log $log_path/$2_dist_log
    ...
    python -u ../../../tools/static_trainer.py -m config_bigdata.yaml \
     -o runner.model_save_path="output_model_textcnn_all_st_cpu" runner.use_gpu=False  > ${log_path}/$2.log 2>&1
    print_info $? $2
else
