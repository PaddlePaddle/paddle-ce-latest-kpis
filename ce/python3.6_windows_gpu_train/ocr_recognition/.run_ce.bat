...
rem train
python train.py --model=attention --total_step=20 --save_model_period=10 --eval_period=10 --save_model_dir=output_attention --use_gpu=True > %log_path%/ocr_attention_T.log 2>&1
if %errorlevel% GTR 0 (
...
python eval.py --model=attention --model_path=output_attention/model_00010 --use_gpu True > %log_path%/ocr_attention_E.log 2>&1
if %errorlevel% GTR 0 (
...
python infer.py --model=crnn_ctc --model_path=output_ctc/model_00100 --use_gpu True --input_images_dir=data/test_images --input_images_list=data/test.list > %log_path%/ocr_ctc_I.log
 if %errorlevel% GTR 0 (
...
python infer.py --model=attention --model_path=output_attention/model_00010 --use_gpu True --input_images_dir=data/test_images --input_images_list=data/test.list > %log_path%/ocr_attention_I.log 2>&1
if %errorlevel% GTR 0 (
