...
%sed% -i s/"max_iterations: 3000000"/"max_iterations: 6"/g configs/waveflow_ljspeech.yaml
%sed% -i s/"save_every: 10000"/"save_every: 3"/g configs/waveflow_ljspeech.yaml
%sed% -i s/"test_every: 2000"/"test_every: 3"/g configs/waveflow_ljspeech.yaml
if exist runs (rd /s /q runs)
python -u train.py --config=./configs/waveflow_ljspeech.yaml --root=data/LJSpeech-1.1  --name=waveflow  --batch_size=4 --use_gpu=true > %log_path%/waveflow_train.log
if not %errorlevel% == 0 (
...
python -u synthesis.py --config=./configs/waveflow_ljspeech.yaml --root=data/LJSpeech-1.1 --name=waveflow --use_gpu=true --output=./syn_audios --sample=0 --sigma=1.0 > %log_path%/waveflow_synthesis.log
if not %errorlevel% == 0 (
