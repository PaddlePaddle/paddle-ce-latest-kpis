...
%sed% -i s/"  max_iterations: 2000000"/"  max_iterations: 2"/g configs/clarinet_ljspeech.yaml
%sed% -i s/"  checkpoint_interval: 1000"/"  checkpoint_interval: 1"/g configs/clarinet_ljspeech.yaml
%sed% -i s/"  eval_interval: 1000"/"  eval_interval: 1"/g configs/clarinet_ljspeech.yaml
if exist experiment (rd /s /q experiment)
if not exist wavenet (mklink /j wavenet %data_path%\Parakeet\wavenet)
python train.py --config=./configs/clarinet_ljspeech.yaml --data=data/LJSpeech-1.1/ --device=-1  --wavenet="./wavenet/step-2" experiment > %log_path%/clarinet_train.log
if not %errorlevel% == 0 (
...
python synthesis.py --config=./configs/clarinet_ljspeech.yaml --data=data/LJSpeech-1.1/ --device=-1  --checkpoint="experiment/checkpoints/step-2" experiment > %log_path%/clarinet_synthesis.log
if not %errorlevel% == 0 (
