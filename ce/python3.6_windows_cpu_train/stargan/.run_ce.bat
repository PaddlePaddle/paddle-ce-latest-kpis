...
python train.py --model_net StarGAN --output output_stargan --dataset celeba --crop_size 178 --image_size 128 --train_list ./data/celeba/list_attr_celeba.txt --test_list ./data/celeba/test_list_attr_celeba.txt  --gan_mode wgan --batch_size 1 --epoch 1 --run_test false --save_checkpoints true --shuffle false --use_gpu false --num_discriminator_time 1 --print_freq 1 --enable_ce | python _ce.py
rem infer
python infer.py --model_net StarGAN --init_model output_stargan/checkpoints/0/ --dataset_dir "data/celeba/" --image_size 128 --c_dim 5 --selected_attrs "Black_Hair,Blond_Hair,Brown_Hair,Male,Young" --use_gpu false > %log_path%/stargan_I.log 2>&1
if not %errorlevel% == 0 (
