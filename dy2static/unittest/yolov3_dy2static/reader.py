...
class DataSetReader(object):
    ...
    def _parse_dataset_catagory(self):
        ...
        if self.num_category != cfg.class_num:
            raise ValueError(
                "category number({}) in your dataset is not equal "
                "to --class_num={} settting, which may incur errors in "
                "eval/infer or cause precision loss.".format(self.num_category,
                                                             cfg.class_num))
    ...
    def get_reader(self,
        ...
        def reader():
            if mode == 'train':
                ...
                batch_out = []
                for img in imgs:
                    im, im_id, im_shape = img_reader(
                        img, size, cfg.pixel_means, cfg.pixel_stds)
...
def train(size=416,
          ...
          use_gpu=True,
          cfg=None):
    dsr = DataSetReader(cfg)
    generator = dsr.get_reader(
        'train', size, batch_size, shuffle, shuffle_seed,
    ...
    def reader():
        ...
        data_loader = fluid.io.DataLoader.from_generator(
            capacity=64, use_multiprocess=True, iterable=True)
        if use_gpu:
            place = fluid.CUDAPlace(fluid.dygraph.parallel.Env(
            ).dev_id) if cfg.use_data_parallel else fluid.CUDAPlace(0)
        else:
            place = fluid.CPUPlace()
        data_loader.set_sample_list_generator(infinite_reader, places=place)
        generator_out = []
        for data in data_loader():
            im, gt_boxes, gt_labels, gt_scores = data[0].numpy(), data[
                1].numpy(), data[2].numpy(), data[3].numpy()
            for i in range(batch_size):
                generator_out.append(
                    [im[i], gt_boxes[i], gt_labels[i], gt_scores[i]])
            yield generator_out
            generator_out = []
            cnt += 1
            if cnt >= total_iter:
