...
class YOLOv3(fluid.dygraph.Layer):
    def __init__(self, cfg, ch_in, is_train=True, use_random=False):
        ...
        self.block = DarkNet53_conv_body(
            ch_in=ch_in, is_test=not self.is_train)
        self.block_outputs = []
        self.yolo_blocks = []
        self.route_blocks_2 = []
        self.cfg = cfg
    ...
    def forward(self,
        ...
        for i, out in enumerate(self.outputs):
            anchor_mask = self.cfg.anchor_masks[i]
            if self.is_train:
                loss = fluid.layers.yolov3_loss(
                    ...
                    class_num=self.cfg.class_num,
                    ignore_thresh=self.cfg.ignore_thresh,
                    downsample_ratio=self.downsample,
                    use_label_smooth=self.cfg.label_smooth)
                self.losses.append(fluid.layers.reduce_mean(loss))

            else:
                mask_anchors = []
                for m in anchor_mask:
                    mask_anchors.append(self.cfg.anchors[2 * m])
                    mask_anchors.append(self.cfg.anchors[2 * m + 1])
                boxes, scores = fluid.layers.yolo_box(
                    x=out,
                    img_size=self.im_shape,
                    anchors=mask_anchors,
                    class_num=self.cfg.class_num,
                    conf_thresh=self.cfg.valid_thresh,
                    downsample_ratio=self.downsample,
                    name="yolo_box" + str(i))
        ...
        if not self.is_train:
            ...
            pred = fluid.layers.multiclass_nms(
                bboxes=yolo_boxes,
                scores=yolo_scores,
                score_threshold=self.cfg.valid_thresh,
                nms_top_k=self.cfg.nms_topk,
                keep_top_k=self.cfg.nms_posk,
                nms_threshold=self.cfg.nms_thresh,
                background_label=-1)
            return pred
        else:
